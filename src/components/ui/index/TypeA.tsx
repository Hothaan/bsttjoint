...
import HoverCard from "./TypeAcard";
...
export default function TypeA() {
  ...
  const baseWidths2 = [35, 40, 25];
  const getHoverWidth = (width: number) => (width > 30 ? 1.2 : 1.4);
  ...
  const hover_card_data1_ = [
    ...
    },
    {
      ...
      hoverText: {
        title: [`20년 연구의 결실,`, <br key="4" />, `3세대 연골한약 백절탕`],
        desc: [
          `튼튼마디의 자부심 백절탕이, 더 높은`,
          <br key="5" />,
        ...
        <div css={card_wrap}>
          <div css={card_inner_wrap}>
            {hover_card_data1_.map((item, idx) => (
              <HoverCard
                key={idx + `index section5 hover card inner1`}
                width={widths1[idx]}
                img={item.img}
                idx={idx}
          ...
          <div css={card_inner_wrap}>
            {hover_card_data2_.map((item, idx) => (
              <HoverCard
                key={idx + `index section5 hover card inner2`}
                width={widths2[idx]}
                img={item.img}
                idx={idx}
                defaultText={item.defaultText}
        ...
        <div css={card_wrap}>
          <div css={card_inner_wrap}>
            {[...hover_card_data1_, ...hover_card_data2_].map((item, idx) => (
              <HoverCard
                key={idx + `index section5 hover card`}
